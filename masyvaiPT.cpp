...
int i = 0;
int studSk = 0;
studentai* stud = nullptr;
bool tikr;
char raide;
while (tikr != 'n')
{
    cout<<"Iveskite studentu skaiciu "<<endl;
    studSk = SkaiciausTikrinimams();
    stud = new studentai[studSk];
    for(studentai* student = stud; student < stud + studSk; student++){
      Ivedimas(*student);
    }
    ...
    if(RaidesTikrinimas() == 'n'){
      ...
      while(raide != 'n'){
        ...
        studentai temp;
        Ivedimas(temp);
        studentai* newStud = new studentai[studSk - 1];
        for(int i = 0; i < studSk - 1; i++){
          newStud[i] = stud[i];
        }
        delete[] stud;
        stud = new studentai[studSk];
        for(int j = 0 ; j < studSk - 1; j++ ){
          stud[j] = newStud[j];
        }
        stud[studSk - 1] = temp;
        delete[] newStud;
        cout<<"Ar bus dar vienas studentas? Iveskite: (t-taip, n-ne)"<<endl;
        raide = RaidesTikrinimas();
        if(raide == 'n'){
        break;
        }
      }
...
double mediana;
double galutinis;
std::sort(A.pazymiai,A.pazymiai + A.pazymiuSk );
  ...
  char ats = RaidesTikrinimas();
      if(ats=='t'){
        ...
        if(ats == 't'){
          for (int j = 0; j < A.pazymiuSk; j++)
          {
            A.pazymiai[j]=distribution(generator);
            cout<<j+1<<"-as sugeneruotas pazymys "<<A.pazymiai[j]<<" ; ";
          }
          cout<<endl;
          ...
          cout<<"Iveskite "<<j+1<< "-a(-i) pazymi:"<<endl;
            A.pazymiai[j] = PazymioTikrinimas();
          }
        ...
        if(ats == 't'){
            while(paz != 'n'){
              A.pazymiai[kiekis] = distribution(generator);
              cout<<kiekis + 1<<"-as sugeneruotas pazymys = "<<A.pazymiai[kiekis]<<endl;
              kiekis++;
          ...
          cout<<"Egzamino sugeneruotas pazymys: "<<A.egzaminas<<endl;
            ...
            A.pazymiai[kiekis] = PazymioTikrinimas();
            kiekis++;
